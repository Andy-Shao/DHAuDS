...
from lib.component import Components, AudioPadding, ReduceChannel, DoNothing, AudioClip, time_shift
...
from HuBERT.CochlScene.train import build_model, inference

def cs_corruption_data(args:argparse.Namespace) -> Dataset:
    if args.corruption_type == 'TST':
        test_set = corrupt_data(
            args=args, orgin_set=CochlScene(
                root_path=args.dataset_root_path, mode='test', include_rate=False, 
                data_tf=Resample(orig_freq=args.org_sample_rate, new_freq=args.sample_rate)
            )
        )
    ...
    test_set = mlt_load_from(
        root_path=dataset_root_path, index_file_name=index_file_name, data_tfs=[ ReduceChannel() ]
    )
...
if __name__ == '__main__':
    ...
    test_set, adapt_set = cs_corruption_data(args)
