...
from lib.component import Components, AmplitudeToDB, FrequenceTokenTransformer, MelSpectrogramPadding
from lib.normAdapt import NormAdapt
from AuT.VocalSound.train import build_model, inference
from AuT.VocalSound.utils import VsAuT
...
def analyzing(args:argparse.Namespace, corruption_types:list[str], corruption_levels:list[str]) -> None:
    ...
    for idx, cmeta in enumerate(corruption_metas):
        ...
        adpt_set = VocalSoundC(
            root_path=args.adpt_set_path, corruption_level=cmeta.level, corruption_type=cmeta.type,
        ...
        eval_set = VocalSoundC(
            root_path=args.eval_set_path, corruption_level=cmeta.level, corruption_type=cmeta.type,
            data_tf=Components(transforms=[
                MelSpectrogram(
                    sample_rate=args.sample_rate, n_fft=n_fft, win_length=win_length, hop_length=hop_length,
                    mel_scale=mel_scale, n_mels=args.n_mels
        ...
        orig_accu = inference(args=args, aut=aut, clsf=clsf, data_loader=eval_loader)

        print('TTN adapting...')
        tp_model = VsAuT(aut=aut, clsf=clsf)
        ttn_model = NormAdapt(model=tp_model, momentum=args.lr_momentum, reset_states=False).to(device=args.device)
        for features, labels in tqdm(adpt_loader):
            features = features.to(args.device)
            ttn_model(features)
        print('Adaptation analyzing...')
        adpt_accu = inference(args=args, aut=aut, clsf=clsf, data_loader=eval_loader)
