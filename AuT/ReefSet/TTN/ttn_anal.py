...
from lib.component import Components, AudioClip, OneHot2Index, AmplitudeToDB, FrequenceTokenTransformer
...
from AuT.ReefSet.utils import RsAuT
...
def analyzing(args:argparse.Namespace, corruption_types:list[str], corruption_levels:list[str]) -> None:
    ...
    for idx, cmeta in enumerate(corruption_metas):
        ...
        orig_roc_auc = inference(args=args, aut=aut, clsf=clsf, data_loader=eval_loader)

        print('TTN adapting...')
        tp_model = RsAuT(aut=aut, clsf=clsf)
        ...
        adpt_roc_auc = inference(args=args, aut=aut, clsf=clsf, data_loader=eval_loader)
