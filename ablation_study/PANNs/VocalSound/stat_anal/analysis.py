...
from lib.component import Components, AudioPadding, ReduceChannel, AudioClip
...
def analyzing(args:argparse.Namespace, corruption_types:list[str], corruption_levels:list[str]) -> None:
    ...
    for idx, cmeta in enumerate(corruption_metas):
        ...
        adpt_set = mlt_load_from(root_path=args.dataset_root_path, index_file_name='meta_info.csv', class_num=args.class_num)
        ...
        orig_accu = inference(args=args, pan=pan, clsf=clsf, data_loader=adpt_loader)
        print('Adaptation analyzing...')
        load_weight(args=args, panns=pan, clsf=clsf, mode='adaptation', metaInfo=cmeta)
        adpt_accu = inference(args=args, pan=pan, clsf=clsf, data_loader=adpt_loader)
...
if __name__ == '__main__':
    ...
    print_argparse(args)
    ##########################################
    analyzing(args=args, corruption_types=['PSTATION'], corruption_levels=['fix']) 