...
from lib.component import Components, ReduceChannel, time_shift, AudioClip, OneHot2Index
...
def build_noise_set(args:argparse.Namespace) -> Dataset:
    ...
    rs_set = ReefSet(root_path=args.dataset_root_path, mode='test', include_rate=False, label_tf=OneHot2Index())
    mlt_store_to(
        dataset=rs_set, root_path=set_path, index_file_name=index_file_name,
...
if __name__ == '__main__':
    ...
    def inferecing(max_roc_auc:float) -> tuple[float, float]:
        ...
        print(f'ROC-AUC is: {val_roc_auc:.4f}, sample size is: {len(noise_set)}')
