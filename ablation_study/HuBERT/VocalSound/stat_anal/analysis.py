...
if __name__ == '__main__':
    ...
    test_loader = DataLoader(
        ...
        dataset=MultiTFDataset(dataset=noise_set, tfs=[ReduceChannel()])
    )

    print('Before adaptation analyzing...')
    load_weight(args=args, hubert=hubert, clsf=clsf, mode='origin')
    b_f1 = inference(args=args, hubert=hubert, clsModel=clsf, data_loader=test_loader)
    print(f'Before adaptation F1 score is: {b_f1:.4f}, sample size is: {len(noise_set)}')
    ...
    a_f1 = inference(args=args, hubert=hubert, clsModel=clsf, data_loader=test_loader)
    print(f'After adaptation F1 score is: {a_f1:.4f}, sample size is: {len(noise_set)}')

    records.loc[len(records)] = [args.dataset, args.arch, 'Fix-corruption', b_f1, a_f1]
